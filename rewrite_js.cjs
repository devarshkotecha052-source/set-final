...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function resetSim() {
    ...
    document.getElementById('packet-1').className = 'packet-sim';
    document.getElementById('packet-2').className = 'packet-sim';
    document.querySelectorAll('.net-node').forEach(n => n.classList.remove('active'));
    
    titleEl.innerText = "Ready to start...";
    whatEl.innerText = "Click Run Simulation to begin the interactive guided journey.";
    whyEl.innerText = "";
    roleEl.innerText = "";
    techEl.innerText = "";
  }
  ...
  btnRunSim.addEventListener('click', async () => {
    ...
    for(let i=0; i<stepsData.length; i++) {
      ...
      if(i === 0) { document.getElementById('packet-1').className = 'packet-sim animate-forward'; }
      if(i === 2) { document.getElementById('packet-2').className = 'packet-sim animate-forward'; }
      if(i === 4) { document.getElementById('packet-2').className = 'packet-sim animate-backward'; }
      
      // Update dev logs
      devLogs.innerText += '\\n> ' + sData.log;
      devLogs.scrollTop = devLogs.scrollHeight;
      
      await sleep(3500); // 3.5 seconds per step so users can read
