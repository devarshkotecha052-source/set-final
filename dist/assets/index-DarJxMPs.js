(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})(),document.addEventListener(`DOMContentLoaded`,()=>{let e=new IntersectionObserver(e=>{e.forEach(e=>{e.isIntersecting&&e.target.classList.add(`visible`)})},{threshold:.1});document.querySelectorAll(`.fade-up, .stagger-item`).forEach(t=>{e.observe(t)});let t=document.querySelectorAll(`.v3-tab:not(.disabled)`),n=document.getElementById(`v3-app-screen`),r=document.getElementById(`v3-diagram-track`),i=document.getElementById(`v3-what`),a=document.getElementById(`v3-why`),o=document.getElementById(`v3-role`),s=document.getElementById(`met-api`),c=document.getElementById(`met-lat`),l=document.getElementById(`met-srv`),u=document.getElementById(`v3-celebration`),d=document.getElementById(`cel-apis`),f=document.getElementById(`cel-time`),p=document.getElementById(`cel-nodes`),m=document.getElementById(`btn-v3-restart`),h=document.getElementById(`btn-next-step`),g=null,_=0,v=0,y=null,b={kyc:{nodes:[{id:`n-app`,icon:`📱`,label:`App Frontend`},{id:`n-srv`,icon:`💻`,label:`App Server`},{id:`n-gw`,icon:`⚡`,label:`Setu Gateway`},{id:`n-auth`,icon:`🔒`,label:`Auth Layer`},{id:`n-rout`,icon:`🔀`,label:`API Router`},{id:`n-nsdl`,icon:`🏛️`,label:`NSDL Gov`}],screens:[{id:`kyc-1`,html:`
        ...
        <div class="v2-timeline-vertical">
          ${r.history.map(e=>`
            <div class="v2-timeline-item">
              <div class="v2-timeline-year">${e.year}</div>
              <div style="font-weight:600; font-size:0.9rem; margin-bottom:4px;">${e.event}</div>
              <div style="font-size:0.8rem; color:#a0aec0;">${e.detail}</div>
            </div>
          `).join(``)}
        </div>
        ...
        <div class="v2-ref-box">
          ${r.references.map(e=>`<div style="margin-bottom:8px;">• ${e}</div>`).join(``)}
        </div>
      `;else if(e===`problem`)t.innerText=`Detailed Problem Study`,n.innerHTML=`
        ...
        <table class="v2-table">
          ...
          <tbody>
            <tr>
              ...
              <td>${r.problem.before.process}</td>
              <td>${r.problem.after.process}</td>
            </tr>
            <tr>
              <td><strong>Time Delay</strong></td>
              <td style="color:#e53e3e;">${r.problem.before.time}</td>
              <td style="color:#48bb78;">${r.problem.after.time}</td>
            </tr>
            <tr>
              <td><strong>Cost Implication</strong></td>
              <td>${r.problem.before.cost}</td>
              <td>${r.problem.after.cost}</td>
            </tr>
            <tr>
              <td><strong>Compliance</strong></td>
              <td>${r.problem.before.regulation}</td>
              <td>${r.problem.after.regulation}</td>
            </tr>
          </tbody>
        </table>
      ...
      `;else if(e===`founders`)t.innerText=`Founder Deep Dives`,n.innerHTML=`
        <div style="margin-bottom:20px; font-size:0.9rem; color:#a0aec0;">
        ...
        ${r.founders.map(e=>`
          <div style="background:rgba(255,255,255,0.02); padding:16px; border-radius:12px; margin-bottom:20px; border:1px solid rgba(255,255,255,0.05);">
            <h3 style="color:#fff; margin-bottom:4px;">${e.name}</h3>
            <div style="font-size:0.8rem; color:var(--primary-color); font-weight:600; margin-bottom:12px;">${e.title}</div>
            <p style="font-size:0.85rem; margin-bottom:12px;">${e.bio}</p>
            <div style="font-size:0.8rem; color:#a0aec0;">
              <strong>Education:</strong> ${e.education}<br>
              <strong>Past Roles:</strong> ${e.previousRoles}<br>
              <strong>Current Status:</strong> ${e.currentRole}
            </div>
          </div>
        `).join(``)}
      ...
      `;else if(e===`business_model`)t.innerText=`Business Canvas & Unit Economics`,n.innerHTML=`
      ...
      `;else if(e===`products`)t.innerText=`Product Suite Deep Dive`,n.innerHTML=`
        <div style="margin-bottom:20px; font-size:0.9rem; color:#a0aec0;">
        ...
        ${r.products.map(e=>`
          <div style="background:rgba(255,255,255,0.02); padding:16px; border-radius:12px; margin-bottom:16px; border:1px solid rgba(255,255,255,0.05);">
            <h4 style="color:#fff; margin-bottom:4px;">${e.name}</h4>
            <div style="font-size:0.8rem; color:var(--primary-color); font-weight:600; margin-bottom:8px;">Launched: ${e.launch}</div>
            <p style="font-size:0.85rem; margin-bottom:8px;">${e.desc}</p>
            <div style="font-size:0.8rem; color:#a0aec0;">
              <strong>Pricing Model:</strong> ${e.pricing}<br>
              <strong>Target Customers:</strong> ${e.customers}
            </div>
          </div>
        `).join(``)}
      `;else if(e===`scale`)t.innerText=`Scale & Ecosystem Map`,n.innerHTML=`
      ...
      `;else if(e===`status`){t.innerText=`Financial & Competitor Matrix`,n.innerHTML=`
      ...
      `;let e=document.getElementById(`revenueChart`).getContext(`2d`),i=r.financials.revenueHistory.map(e=>e.year),o=r.financials.revenueHistory.map(e=>parseFloat(e.revenue.replace(`INR `,``).replace(` Cr`,``))),s=r.financials.revenueHistory.map(e=>parseFloat(e.loss.replace(`INR `,``).replace(` Cr`,``)));a=new Chart(e,{type:`line`,data:{labels:i,datasets:[{label:`Revenue (INR Cr)`,data:o,borderColor:`#42cacd`,backgroundColor:`rgba(66,202,205,0.1)`,borderWidth:2,fill:!0},{label:`Losses (INR Cr)`,data:s,borderColor:`#e53e3e`,backgroundColor:`rgba(229,62,62,0.1)`,borderWidth:2,fill:!0}]},options:{responsive:!0,scales:{y:{grid:{color:`rgba(255,255,255,0.05)`},ticks:{color:`#a0aec0`}},x:{grid:{color:`rgba(255,255,255,0.05)`},ticks:{color:`#a0aec0`}}},plugins:{legend:{labels:{color:`#fff`}}}}})}}});